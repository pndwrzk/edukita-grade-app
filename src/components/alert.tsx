...
const Alert: React.FC<Props> = ({ message, isSuccess }) => {
  return (
    <div className="fixed inset-x-0 top-0 flex items-end justify-center px-4 py-6">
      <div
        className={`${
          isSuccess ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"
        } px-6 py-4 rounded-md text-lg flex items-center max-w-lg shadow-lg`}
      >
        {isSuccess ? (
          <svg
            viewBox="0 0 24 24"
          ...
          <svg
            viewBox="0 0 24 24"
            className="text-red-600 w-5 h-5 mr-3"
          >
            <path
              fill="currentColor"
...
export const showAlert = (message: string, isSuccess: boolean) => {
